...
const metadata = {
    ...
    i18n: {
        "default": {
            ...
            pausedSuccessfully: "🤚 Paused the current song",
...
async function execute(ctx, interaction) {
    ...
    if (!interaction.member?.voice?.channel && !interaction.guild?.members?.voice?.channel) {
        ...
        await interaction.editReply({ content: metadata.i18n[`${metadata.i18n[interaction.locale] ? interaction.locale : "default"}`].pausedSuccessfully.replace("%s", interaction.member?.voice?.channel) });
    };
};
